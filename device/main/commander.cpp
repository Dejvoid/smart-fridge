...
void Commander::loop() {
    ...
    if (scan_on) {
        ...
        if (fb == NULL) {
            ...
            if (cam_->scan_code(msg_.data)) {
                ...
                mqtt_->publish(msg_);
                scan_on = false;
                lcd_->draw_text("Scanned: " + msg_.data, 0, frame_h + LcdDriver::font_size, 0xff, 0xff, 0xff);
            }
        }
        cam_->ret_frame();
    }
    // Draw temp and hum on display
    ...
    std::string temp_str = "Temp: " + std::to_string(temp_) + "°C";
    std::string hum_str = "Hum: " + std::to_string(hum_) + "%";
